...
from TF_model import TFNet


def total_loss(input, target, weights=[0.6, 0.3, 0.1]):
    """
    ...
    for i in range(n):
        ...
        dice_loss = loss.MulticlassDiceLoss()(logits, rescale_target, weights=[0.4, 0.2, 0.4])
...
if __name__ == '__main__':
    ...
    dict_path = r'/home/wyd/PycharmProjects/ZWW_seg/data/High_index/dict_index'
    data_modes = ['qsm.npy']
    roi_modes = ['ROI.npy']
    input_shape = (96, 96)
    TF = True
    ...
    if TF:
        tf_model_path = r'/home/wyd/PycharmProjects/ZWW_seg/Low_model/log/model/Epoch244_best_model.pth'
        network = TFNet(tf_model_path)
    else:
        in_ch = 3
        n_classes = 4
    ...
    if len(device_ids) > 1:
        ...
        network = network.cuda(device=device_ids[0])
    else:
        network = network.cuda(device=device_ids[0])
    ...
    train_val(network, loaders, optimizer, criterion, early_stopping, device_ids, log_path, loss_list, n_epochs=300)
